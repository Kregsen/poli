...
class pme:
    ...
    def formata_poisson(poi_res):
        # Formata o resultado obtido pela função poisson_vp(omega, omega_vrs, dist, dist_vrs, vq, vq_vrs).

        if pme.teste_numero(poi_res[0]):
            poi_res[0] = str(poi_res[0])
            poi_f1 = "".join(poi_res)
            return f"\nA velocidade por Poisson é: V(P) = {poi_f1}\n" 
        
        else:
            poi_f2 = poi_res[0] + poi_res[1]
            return f"\nA velocidade por Poisson é: V(P) = {poi_f2[0]}{poi_f2[2]} + {poi_f2[1]}{poi_f2[3]}\n" 
